...
export default class BoardController {
  public static async GetUsersByID(
    ...
    for (let userID of usersID) {
      users.push(
        (await UserModel.findById(userID).select('pseudo picture')) as IUser
      );
    }
    return users;
  ...
  public static async create(req: any, res: Response) {
    ...
    try {
      ...
      if (boardPicture) {
        pictureName = await FileManager.uploadPicture(
          name,
          'board-picture',
          boardPicture
        );
  ...
  public static async getAllBoards(req: Request, res: Response) {
    console.log('getAllBoards');

    const userID = req.params.id;

    const userBoards = Utils.toObject(
      await UserModel.findById(userID).select('boards -_id')
    );

    const boards: Array<any> = await Utils.toObject(
      await BoardModel.find({ isPrivate: false })
    );

    for (let i = 0; i < boards.length; i++) {
      const boardMembers = await UserModel.find({
  ...
  public static async getBoard(req: Request, res: Response) {
    ...
    try {
      ...
      await BoardModel.findById(
        id,
        async (err: any, docs: MongooseDocument) => {
          try {
  ...
  public static async sendBoardInvitation(
    ...
    invitations.forEach(async (invit) => {
      invit.notifications = await invit.notifications[
        invit.notifications.length - 1
      ];
    });
  ...
  public static async banMember(boardID: string, memberBannedID: string) {
    await UserModel.findByIdAndUpdate(memberBannedID, {
      $pull: { boards: boardID },
    });
    await BoardModel.findByIdAndUpdate(boardID, {
  ...
  public static async getAvailableAssignedMembers(req: Request, res: Response) {
    ...
    const card: ICard = await ListController.getCardFromDB(
      boardID,
      listID,
      cardID
    );
